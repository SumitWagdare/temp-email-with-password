...
export class OneSecMailProvider implements EmailProvider {
  ...
  async getDomains(): Promise<string[]> {
    ...
    if (!res.ok) {
      throw new Error(`1secmail getDomains error: ${res.statusText}`);
    }
  ...
  async getMessages(account: InboxAccount): Promise<EmailMessage[]> {
    ...
    if (!res.ok) {
      throw new Error(`1secmail getMessages failed: ${res.statusText}`);
    }
  ...
  async getMessageDetail(account: InboxAccount, messageId: string): Promise<EmailDetail> {
    ...
    if (!res.ok) {
      throw new Error(`1secmail readMessage failed: ${res.statusText}`);
    }
